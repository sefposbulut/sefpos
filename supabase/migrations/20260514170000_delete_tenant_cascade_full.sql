-- delete_tenant_cascade: super-admin/owner'in tum tenant verisini guvenle silmesi
-- icin tam temizlik akisi. Her zaman SECURITY DEFINER.
-- Eski surumde eksik olan tablolar (waiters, waiter_calls, device_bindings,
-- order odemeleri vs) eklendi.
--
-- 404 sebebi: PostgREST schema cache eski. CREATE OR REPLACE + NOTIFY ile yenilenir.
...
BEGIN
  -- Yetki kontrolu: super_admin veya tenant'in kendi owner'i tetikleyebilir
  IF v_caller IS NULL THEN
    -- service_role direkt baglandiginda auth.uid() null olur; bu durumda izin ver
    NULL;
  ELSE
  ...
  -- 1) Order'a bagli kayitlar
  DELETE FROM public.order_items oi
  WHERE oi.order_id IN (SELECT id FROM public.orders WHERE tenant_id = p_tenant_id);
  ...
  -- credit_transactions tablosu varsa
  ...
  IF to_regclass('public.restaurant_tables') IS NOT NULL THEN
    DELETE FROM public.restaurant_tables
     WHERE table_group_id IN (SELECT id FROM public.table_groups WHERE tenant_id = p_tenant_id);
  END IF;
  ...
  -- 9) Profiller (auth.users CASCADE ile düşmez; bu fonksiyon SADECE public şemayı temizler.
  --     auth.users temizlemek için ayrıca update-user edge fonksiyonunu çağırın.)
...
COMMENT ON FUNCTION public.delete_tenant_cascade(uuid) IS
  'Tenant geneli kademeli temizlik. super_admin veya tenant owner/admin tetikleyebilir.';

-- PostgREST schema cache reload (404 onlemi)
